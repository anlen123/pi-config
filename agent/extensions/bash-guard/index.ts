...
export default function (pi: ExtensionAPI) {
	...
	pi.registerFlag("bash-guard-interactive", {
		description: "Start with bash-guard in interactive mode (prompts for risky commands). Default is autonomous mode.",
		type: "boolean",
		default: false,
	});

	// Default: autonomous (disabled) mode — no interactive prompts, catastrophic floor still applies.
	// Toggle with /bash-guard to switch to interactive (prompting) mode for this session.
	// Session-local. Intentionally not persisted across reloads or restarts.
	let disabled = true;

	pi.on("session_start", async (event, ctx) => {
		if (event.reason !== "startup") return;
		if (pi.getFlag("--bash-guard-interactive") === true) {
			// Opt into interactive (prompting) mode for this session.
			disabled = false;
			return;
		}
		// Default (and legacy --bash-guard-disabled): autonomous mode with catastrophic floor.
		disabled = true;
		const { theme } = ctx.ui;
	...
	pi.registerCommand("bash-guard", {
		description: "Toggle bash-guard between autonomous (default, no prompts) and interactive (prompting) for this session.",
		handler: async (_args, ctx) => {
			...
			if (disabled) {
				...
				ctx.ui.setStatus(BASH_GUARD_STATUS_KEY, badge);
				ctx.ui.notify(
					"bash-guard: autonomous mode. Catastrophic operations are still hard-blocked. Run /bash-guard again for interactive prompts.",
					"warning",
				);
			} else {
				ctx.ui.setStatus(BASH_GUARD_STATUS_KEY, undefined);
				ctx.ui.notify("bash-guard: interactive mode — risky commands will prompt for confirmation.", "info");
			}
		},
	});
